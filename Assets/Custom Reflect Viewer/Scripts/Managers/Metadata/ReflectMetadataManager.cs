...
namespace UnityEngine.Reflect.Extensions
{
    ...
    public class ReflectMetadataManager : MonoBehaviour, INotifyBIMObservers, IManageMetadata
    {
        ...
        [Tooltip("If you want to load saved models with Addressables system and not use Reflect runtime menu.")]
        [SerializeField] bool loadSavedModels = false;
        /// <summary>
        ...
        void Awake()
        {
            ...
            if (syncManager == null)
            {
                Debug.LogWarning("WARNING: There is no Reflect SyncManager which is fine if model is attached already to Reflect Root object or loading from saved asset bundles.");
            }
            else
            {
                if (reflectRoot == null)
        ...
        void CheckForModel()
        {
            if (reflectRoot != null && loadSavedModels)
            {
                // Using Addressables to load models
                SetMetadataBehavior(new SavedModelsMetadataBehavior(instance));
                // Call for the project list
                AddressablesManager.Instance.LoadAvailableModelList();
            }
            else if (reflectRoot != null && reflectRoot.childCount > 0)
                // Model is in the scene
